 ...
 * @param  {} next
 */
module.exports.getCustodianPosition = async (req, res, next) => {
	try {
		const axios = require("axios");
		const dateFormat = require("date-format");
		...
		const return_data = {
			cash: (!data.data.data.cash) ? [] : data.data.data.cash.map((r) => ({
				"Currency": r.ccy,
				"Balance": numeral(r.balance).format("0,0.[0000000000]"),
			})),
			bond: (!data.data.data.bond) ? [] : data.data.data.bond.map((r) => ({
				"Product Code": r.isin,
				"Product Name": r.productname,
				"Currency": r.ccy,
				"Nominal": numeral(r.nominal).format("0,0.[0000000000]"),
				"Reference Price": numeral(math.multiply(r.ref_price, 100)).format("0,0.[0000000000]") + "%",
				"Reference Value": numeral(r.ref_value).format("0,0.[0000000000]"),
			})),
			rp: (!data.data.data.rp) ? [] : data.data.data.rp.map((r) => ({
				"Product Code": r.isin,
				"Product Name": r.productname,
				"Currency": r.ccy,
				"Nominal": numeral(r.nominal).format("0,0.[0000000000]"),
			...
			rs: (!data.data.data.rs) ? [] : data.data.data.rs.map((r) => ({
				"Product Code": r.isin,
				"Product Name": r.productname,
				"Currency": r.ccy,
				"Nominal": numeral(r.nominal).format("0,0.[0000000000]"),
 ...
 * @param  {} next
 */
module.exports.getCustodianChargeOffHistory = async (req, res, next) => {
	try {
		const axios = require("axios");
		const dateFormat = require("date-format");
		const config = require("../Config");
		...
		const return_data = {
			cash: (!data.data.data.cash) ? [] : data.data.data.cash.map((r) => ({
				...
				"Currency": r.ccy,
				"Amount": numeral(r.amount).format("0,0.[0000000000]")
			})),
			holdings: (!data.data.data.bond) ? [] : data.data.data.bond.map((r) => ({
				"Trade Date": dateFormat.asString("yyyy/MM/dd", new Date(r.tradedate)),
				"Value Date": dateFormat.asString("yyyy/MM/dd", new Date(r.valuedate)),
				"Buy/Sell": r.buysell,
 ...
 * @param  {} next
 */
module.exports.getCustodianNotChargeOffHistory = async (req, res, next) => {
	try {
		const axios = require("axios");
		const dateFormat = require("date-format");
		const config = require("../Config");
